...
const app = {
  ...
  renderHomeRecentWords() {
    ...
    if (this.words.length === 0) {
      container.innerHTML = '<p class="empty-text">등록된 단어가 없다. 새로운 단어를 추가해라!</p>';
      return;
    }
  ...
  handleWordSubmit(event) {
    ...
    // Show Toast
    this.showToast('새로운 단어 등록 완료 잇힝!');
  ...
  loadSampleData() {
    ...
    this.saveData();
    this.setupQuizRequirements();
    this.showToast(`${addedCount}개의 기초 단어가 추가되었다!`);
  },
  ...
  setupQuizRequirements() {
    ...
    if (this.words.length >= 4) {
      requirementsText.innerHTML = '퀴즈를 시작할 수 있음!';
  ...
  renderQuestion() {
    ...
    let label = '';
    if (currentQ.answerField === 'meaning') label = '알맞은 [뜻] 고르기';
    else if (currentQ.answerField === 'hiragana') label = '알맞은 [발음] 고르기';
    else if (currentQ.answerField === 'kanji') label = '알맞은 [한자] 고르기';
  ...
  restartQuizAfterFinished() {
    ...
    activeStateContainer.innerHTML = `
      ...
      <div class="question-card">
        <div class="question-type-badge" id="question-type-label">뜻 맞추기</div>
  ...
  async clearAllWordsWithConfirm() {
    ...
    if (confirm('다 삭제해? 복구 불가인데?')) {
      ...
      this.filterWords();
      this.updateStatsUI();
      this.showToast('텅 비었습니다. 마치 니 머리처럼.');
    }
  },
